...
const { percentageToValue } = require('../../../utils/utils');
...
function command(deviceFeature, value) {
  ...
  switch (type) {
    ...
    case DEVICE_FEATURE_TYPES.LIGHT.COLOR:
      code = 0x0d;
      message = [value == 0 ? 2 : 1, value % 256, Math.floor(value / 256) % 256, Math.floor(value / 65536) % 256, 0, 0];
      break;
    case DEVICE_FEATURE_TYPES.LIGHT.TEMPERATURE:
      ...
      break;
    default:
      throw new Error(`AwoX - Legacy: ${type} feature not managed`);
  }
